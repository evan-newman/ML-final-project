...
class Net1(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(1053, 500)
        self.fc1_drop = nn.Dropout(0.2)
        self.fc2 = nn.Linear(500, 2)

    def forward(self, x):
        x = x.view(-1, 1*1053) #One column representing all rgb bits
...
def load_train_data():
    ...
    for lineNumber, line in enumerate(f):
        if lineNumber != 0:
            entries = line.split('\t')
            data.append(list(map(float, entries[2:1055])))
    ...
    for idx in range(len(trainData)):
        ...
        if (idx + 1) % BATCH_SIZE == 0:
            ...
            x = np.array(batchX).reshape((BATCH_SIZE, 1, 1053))
...
def load_test_data():
    ...
    for lineNumber, line in enumerate(f):
        if lineNumber != 0:
            entries = line.split('\t')
            data.append(list(map(float, entries[1:1054])))
            id.append(entries[0])
...
def main():
    ...
    #print(prob_arr_epoch[4])
    #print(training_mat[0].pred[0][1].item())
    print_file(testing_mat,"featuresall_pred1.txt")
    #print_file(training_mat, "results_train1053.txt")
